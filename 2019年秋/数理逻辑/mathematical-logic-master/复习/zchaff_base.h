...
typedef int ClauseIdx;  // 用于标识子句   Used to refer a clause. Because of dynamic     用于标识子句
...
//
  // Description [A literal is a variable with phase. Two things specify a
			  ...
			  // 文字是带有相位的变量。两件事说明了字面意思:/它的“符号”和它的可变索引。
			   ...
			   //每个超过1个文本的子句包含两个特殊的文字。他们叫做“被监视的文字”。
			   // 文字标记占用2位内容:  00->未观看；11->观看，方向= 1； 01->观看，目录=-1；   10无效。这两位占据了文字的最低有效位
			   ...
			   // 每个文字由一个32位有符号整数表示。这更高的29位代表可变索引。最多2**28 允许变量。
			   //如果这个整数的符号是否定的，这意味着它不是有效的文字。有可能是子句索引或删除的文字池元素。
			   // 第三次最低有效位用于标记其符号。 0->正，1->负。
               ...
               // index.]
			   //文本被收集在一个名为文本的存储空间中泳池。
			   //文本池中的元素可以是文本或表示终止的特殊间隔元素子句。
			   //间隔元素具有子句的负值索引。
//
...
//
  // Description [A clause is consisted of a certain number of literals.
               ...
               // position of it's literals in the pool.
				// 子句由一定数量的文字组成。所有文字都被收集在一个称为文字池。每个子句都有一个指向开头的指针它的文字在池中的位置。
			   ...
			   //子句可以在搜索过程中由冲突驱动生成分析。冲突条款由解决方案生成过程
			   // 因此，如果经过一次搜索，一些子句得到删除后，一些学习到的冲突条款可能会被无效。
			   //为了保持条款的完整性数据库中，有必要跟踪参与某一冲突的解决过程这样当这些条款被删除时，冲突条款也应删除。
...
class CVariable {
  protected:
    unsigned _value             : 2;  // it can take 3 values, 0, 1 and UNKNOWN    变量的取值情况有三种：0、1、unknown
    bool _marked                : 1;  // used in conflict analysis.                冲突分析
    unsigned _new_cl_phase      : 2;  // it can take 3 value                       用于跟踪新增子句中出现的文字
    ...
    // a. each variable can only appearing in one phase                            每个变量只能出现在一个阶段
    // b. same literal won't appear more than once.                                相同的文字不会多次出现
